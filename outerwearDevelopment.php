...
<body>
    ...
    <style>
        header .menu__content #grimmNavbar .navbar-nav .nav-item .outerwear__item {
    ...
    if ($search) {
        $products = mysqli_query($con, "select * from `sanpham` where MALOAI = 3 AND `TENSP` LIKE '%" . $search . "%'  " . $orderCondition . "  LIMIT " . $item_per_page . " OFFSET " . $offset . " ");
        $totalRecords = mysqli_query($con, "select * from `sanpham` where MALOAI = 3 AND `TENSP` LIKE '%" . $search . "%'");
    } else {
        $products = mysqli_query($con, "select * from `sanpham` WHERE MALOAI = 3 " . $orderCondition . "  LIMIT " . $item_per_page . " OFFSET " . $offset . " ");
        $totalRecords = mysqli_query($con, "select * from `sanpham` where MALOAI=3");
    }
