...
<body>
    ...
    if ($search) {
        $products = mysqli_query($con, "select * from `sanpham` where MALOAI = 2 AND `TENSP` LIKE '%" . $search . "%'  " . $orderCondition . "  LIMIT " . $item_per_page . " OFFSET " . $offset . " ");
        $totalRecords = mysqli_query($con, "select * from `sanpham` where MALOAI = 2 AND `TENSP` LIKE '%" . $search . "%'");
    } else {
        $products = mysqli_query($con, "select * from `sanpham` WHERE MALOAI = 2 " . $orderCondition . "  LIMIT " . $item_per_page . " OFFSET " . $offset . " ");
